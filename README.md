...
> **一句话概括**：PM 产出任务卡并派发，角色（独立对话）完成后写回报到仓库，PM 验收形成闭环；重要决策节点（角色组合确认、原型确认、产品验收、需求变更代价确认）由用户决定。派发/回报在 DSH 等可靠宿主下默认自动化编排；无可靠子代理能力的宿主退回手工模式（用户当信使）。
>
> **本仓库是框架模板**：`docs/` 中的待填项（如 `[待 PM 初始化时填写]`、`YYYY-MM-DD`）是复制到新项目后由 PM 初始化的占位，不是未完成内容。
